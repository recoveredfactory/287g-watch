// Shared homepage data builder. The homepage (routes/+page.server.ts) and the
// social-video route (routes/video/national/+page.server.ts) both render the
// same map + trend chart, so they build the identical `PageData` here once —
// no duplicated trend logic, and the video's headline numbers can't drift from
// the site's. Server-only (the `/server/` segment keeps it out of the client
// bundle); the pure shapes live in $lib/homeData.types.
...
export const buildHomeData = async (
