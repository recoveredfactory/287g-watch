...
function transformCoord([lng, lat], { srcCenter, tgtCenter, scale, yScale }) {
  const lng2 = fixLng(lng);
  return [
    tgtCenter[0] + scale * (lng2 - srcCenter[0]),
    tgtCenter[1] + (yScale ?? scale) * (lat - srcCenter[1]),
  ];
}
...
export const INSET_TRANSFORMS = {
  // yScale compensates for Mercator distortion: AK moves from 63.5°N to 25.5°N,
  // so lat diffs need to grow by cos(25.5°)/cos(63.5°) ≈ 2.02 to preserve shape.
  AK: { srcCenter: [-153.5, 63.5], tgtCenter: [-118.5, 25.5], scale: 0.29, yScale: 0.587 },
