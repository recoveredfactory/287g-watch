...
<main id="main-content" class="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
  ...
  <h1 class="text-2xl font-black text-ink-900 sm:text-4xl">{m.glossary_heading()}</h1>
  <p class="prose-editorial mt-3 max-w-xl">
    {m.glossary_subtitle()}
  </p>

  <!-- Terms -->
  <dl class="mt-6 space-y-6 border-t border-paper-200 pt-6 sm:mt-8">
    {#each terms as t}
      <div id="term-{termSlug(t.term)}">
        <dt class="font-semibold text-ink-900">{t.term}</dt>
